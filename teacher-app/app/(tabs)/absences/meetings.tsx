...
export default function MeetingAbsencesScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await api.get('/api/meetings/my-absences');
      ...
      {loading
        ...
        ? (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyTitle}>No meeting absences</Text>
            <Text style={styles.emptySub}>You have attended all recorded meetings</Text>
          </View>
        )
        : absences.map(ab => (
          <View key={ab.id} style={styles.card}>
            <View style={styles.cardRow}>
              <View style={styles.flex1}>
                <Text style={styles.title}>{ab.meeting_title}</Text>
                <Text style={styles.type}>{formatType(ab.meeting_type)}</Text>
...
const styles = StyleSheet.create({
  ...
  type:            { fontSize: 11, fontWeight: '600', color: '#8C7E6E', marginTop: 1 },
