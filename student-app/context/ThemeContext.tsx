...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setPrimary(p); setAccent(a); setIsDark(dark);
      setColors(buildColors(p, a, dark));
      setLoaded(true);
    });
  }, [systemScheme]);

  const updateTheme = useCallback(async (newP: string, newA: string) => {
    await storage.saveTheme(newP, newA);
    setPrimary(newP); setAccent(newA);
    setColors(buildColors(newP, newA, isDark));
  }, [isDark]);
