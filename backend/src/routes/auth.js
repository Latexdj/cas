  ...
  try {
    ...
    const { rows } = await pool.query(
      `SELECT id, name, email, password_hash, is_active
       FROM library_staff WHERE school_id = $1 AND LOWER(email) = LOWER($2)`,
      [school.id, email]
    );
    if (!rows.length) return res.status(401).json({ error: 'Invalid email or password' });
    const staff = rows[0];
    if (!staff.is_active) return res.status(401).json({ error: 'Account deactivated. Contact your administrator.' });

    const valid = await bcrypt.compare(String(password), staff.password_hash);
    if (!valid) return res.status(401).json({ error: 'Invalid email or password' });

    const token = jwt.sign(
      { id: staff.id, name: staff.name, role: 'library_staff', schoolId: school.id },
...
// ── POST /api/auth/clearance-login ───────────────────────────────────────────
// Dedicated login for non-teaching clearance staff
router.post('/clearance-login', loginLimiter, async (req, res, next) => {
  try {
    const { email, password, schoolCode } = req.body;
    if (!email || !password || !schoolCode) {
      return res.status(400).json({ error: 'email, password and schoolCode are required' });
    ...
    const { rows: schoolRows } = await pool.query(
      `SELECT id, name, primary_color, accent_color, logo_url
       FROM schools WHERE UPPER(school_code) = UPPER($1)`,
      [schoolCode]
    );
    if (!schoolRows.length) return res.status(404).json({ error: 'School not found' });
    const school = schoolRows[0];
    ...
    const { rows } = await pool.query(
      `SELECT id, name, email, password_hash, is_active
       FROM clearance_staff WHERE school_id = $1 AND LOWER(email) = LOWER($2)`,
      [school.id, email]
    );
    if (!rows.length) return res.status(401).json({ error: 'Invalid email or password' });
    const staff = rows[0];
    ...
    const token = jwt.sign(
      { id: staff.id, name: staff.name, role: 'clearance_staff', schoolId: school.id },
      process.env.JWT_SECRET,
      { expiresIn: '12h' }
    );
