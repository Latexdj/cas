...
// Only teachers or clearance_staff can use these routes
function clearanceStaffOnly(req, res, next) {
  const role = req.user?.role;
  if (role !== 'teacher' && role !== 'admin' && role !== 'clearance_staff') {
...
async function getMyOfficeIds(schoolId, userId, role) {
  if (role === 'clearance_staff') {
    const { rows } = await pool.query(
      `SELECT office_id FROM clearance_office_staff
       WHERE school_id = $1 AND clearance_staff_id = $2`,
      [schoolId, userId]
    );
