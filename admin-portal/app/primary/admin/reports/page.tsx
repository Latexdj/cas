...
export default function PrimaryReportsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSummary(data);
    } catch { setError('Could not load report overview.'); }
  ...
  async function drillDown(className: string) {
    ...
    } catch { setError('Could not load class detail.'); }
      ...
      <div className="bg-white rounded-xl border border-slate-200 px-4 py-3 flex flex-wrap gap-3 shadow-sm">
        ...
        {drillClass && (
          <button onClick={() => { setDrillClass(''); setClassReports([]); }}
            className="px-3 py-1.5 text-sm text-slate-600 border border-slate-200 rounded-lg hover:bg-[#F5F0E8]">
            ← All Classes
          </button>
        )}
      </div>
          ...
          {summary.map(cls => {
            ...
            return (
              <button key={cls.class_name} onClick={() => drillDown(cls.class_name)}
                className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 text-left hover:border-[#8FC4A4] hover:shadow-md transition-all">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                  <div className="text-center p-1.5 rounded-lg bg-[#F5F0E8]">
                  ...
                  <div className="text-center p-1.5 rounded-lg bg-[#E8F4EE]">
                    <p className="font-bold text-[#145C44]">{cls.approved_count}</p>
                    <p className="text-[#2D7A4F]">Approved</p>
                  </div>
                ...
                <div className="mt-3">
                  ...
                  <div className="h-1.5 rounded-full bg-slate-100 overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${approvedPct}%`, backgroundColor: '#145C44' }} />
                  </div>
                </div>
              </button>
            );
                    ...
                    {r.status === 'submitted' && r.report_id && (
                      <div className="flex gap-2">
                        <button onClick={() => openAction(r.report_id!, 'approve')}
                          className="text-xs px-2.5 py-1 rounded-md border border-[#B8D9C8] text-[#145C44] hover:bg-[#E8F4EE] font-semibold">
                          Approve
                        </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0B3D2E]/45 px-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button onClick={() => setShowPanel(null)} className="px-4 py-2 rounded-lg text-sm font-semibold text-slate-700 border border-slate-200 hover:bg-[#F5F0E8]">Cancel</button>
              <button onClick={() => doAction(showPanel)} disabled={actioning}
                ...
                style={showPanel === 'approve' ? { backgroundColor: '#145C44' } : {}}>
