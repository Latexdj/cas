...
function SchedulesTab({
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <p style={{ color: '#64748b', fontSize: 14 }}>Set fee amounts per class and term, then generate bills for students.</p>
                ...
                <td style={{ padding: '10px 12px' }}>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    <button style={{ ...btnPrimary, padding: '5px 10px', fontSize: 12 }}
                      onClick={() => generate(s.id)} disabled={generating === s.id}>
  ...
  return (
    <div>
      ...
      <div style={{ position: 'relative', maxWidth: 420, marginBottom: 20 }}>
        ...
        {results.length > 0 && (
          <div style={{
            position: 'absolute', top: 38, left: 0, right: 0, background: '#fff',
            border: '1px solid #e2e8f0', borderRadius: 8, boxShadow: '0 4px 16px rgba(0,0,0,.1)', zIndex: 20,
          }}>
            {results.map(s => (
              <div key={s.id} style={{ padding: '10px 14px', cursor: 'pointer', borderBottom: '1px solid #f1f5f9' }}
            ...
            <tbody>
              ...
              {selected.payments.map(p => (
                <tr key={p.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  ...
                  <td style={{ padding: '8px 10px' }}>
                    <button style={btnDanger} onClick={() => voidPayment(p.id)}>Void</button>
                  </td>
                </tr>
              ))}
            </tbody>
...
export default function FeesPage() {
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 1100 }}>
      ...
      {/* Tabs */}
      <div style={{ borderBottom: '2px solid #e2e8f0', marginBottom: 24, display: 'flex', gap: 4 }}>
        {TABS.map(t => (
