            ...
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                {['Teacher', 'Subject', 'Class', 'Semester', 'Lessons', 'Assignments', 'Quizzes', 'Status', 'Actions'].map(h => (
                  <th key={h} className="px-4 py-3 text-xs font-semibold text-slate-500 font-medium">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                    ...
                    <div className="flex items-center gap-2">
                      {c.status !== 'archived' && (
                        <button
                          ...
                          className="text-xs font-medium text-[#145C44] hover:text-green-900 disabled:opacity-40"
                        >
