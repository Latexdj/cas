...
const STATUS_STYLE = {
  cleared:     { dot: 'bg-[#145C44]',  text: 'text-[#145C44]',  bg: 'bg-[#E8F4EE]',  label: 'Cleared'       },
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <button onClick={() => { setShowBatch(true); setBatchResult(null); setBatchError(''); setBatchClasses([]); }}
          className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold bg-[#145C44] text-white hover:bg-[#145C44]">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 flex flex-wrap gap-3 items-end">
        <div className="flex-1 min-w-[200px]">
          <label className="text-xs font-semibold text-slate-500 font-medium block mb-1">Search</label>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Name or student ID…"
            className={`${sel} w-full`} />
        </div>
        <div>
          <label className="text-xs font-semibold text-slate-500 font-medium block mb-1">Class</label>
        ...
        <div>
          <label className="text-xs font-semibold text-slate-500 font-medium block mb-1">Status</label>
                  ...
                  <tr key={s.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-center">
                      {!s.clearance_id ? (
                        ...
                        <span className="text-xs text-[#145C44] bg-[#D1EAD9] px-2 py-1 rounded-full font-semibold">Fully Cleared</span>
                      ...
                      {s.clearance_id ? (
                        <div className="flex items-center gap-2">
                          <div className="flex-1 bg-slate-100 rounded-full h-2 min-w-[60px]">
                            <div className="h-2 rounded-full bg-[#145C44] transition-all" style={{ width: `${pct}%` }} />
                          </div>
                    ...
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => loadDetail(s.id)}
                        className="text-xs font-semibold text-[#145C44] hover:text-green-900 px-3 py-1.5 rounded-lg hover:bg-[#E8F4EE]">
                        View →
                      </button>
                    </td>
                  </tr>
      ...
      {(detail || detailLoading) && (
        <div className="fixed inset-0 z-50 flex justify-end bg-[#0B3D2E]/45" onClick={() => setDetail(null)}>
            ...
            {detailLoading ? (
              ...
              <div className="p-5 space-y-3">
                {detail.clearance.map(item => {
                  ...
                  return (
                    <div key={item.id} className={`rounded-xl border p-4 ${item.status === 'cleared' ? 'border-[#D1EAD9] bg-[#E8F4EE]' : item.status === 'not_cleared' ? 'border-red-100 bg-red-50' : 'border-slate-100 bg-white'}`}>
            ...
            <div className="px-6 py-4 space-y-4">
              {batchResult ? (
                <div className="space-y-3">
                  ...
                  <button onClick={() => setShowBatch(false)} className="w-full py-2.5 rounded-xl text-sm font-semibold bg-[#145C44] text-white hover:bg-[#145C44]">Done</button>
                </div>
              ) : (
                <>
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    <button onClick={handleBatchInitiate} disabled={initiating || batchClasses.length === 0}
                      className="flex-1 py-2.5 rounded-xl text-sm font-semibold bg-[#145C44] text-white hover:bg-[#145C44] disabled:opacity-50 flex items-center justify-center gap-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0B3D2E]/55 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm">
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              <div>
                <label className="text-xs font-semibold text-slate-500 font-medium block mb-2">Status</label>
                <div className="flex gap-2">
                  {(['pending', 'cleared', 'not_cleared'] as const).map(s => (
                    <button key={s} onClick={() => setOvStatus(s)}
                      className={`flex-1 py-2 rounded-xl text-xs font-bold capitalize border transition-colors ${ovStatus === s ? 'bg-[#145C44] text-white border-green-600' : 'bg-white text-slate-600 border-slate-200 hover:border-[#8FC4A4]'}`}>
                      {s.replace('_', ' ')}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <textarea value={ovNotes} onChange={e => setOvNotes(e.target.value)} rows={3}
                  ...
                  className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#145C44] resize-none" />
              </div>
              <div className="flex gap-3">
                ...
                <button onClick={submitOverride} disabled={ovSaving || (ovStatus === 'not_cleared' && !ovNotes.trim())}
                  className="flex-1 py-2.5 rounded-xl text-sm font-semibold bg-[#145C44] text-white hover:bg-[#145C44] disabled:opacity-50">
