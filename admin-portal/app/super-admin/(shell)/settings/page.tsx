...
export default function SettingsPage() {
  ...
  return (
    <div className="p-6 max-w-lg mx-auto">
      ...
      {/* Security info */}
      <div className="bg-[#145C44]/20 border border-[#B8D9C8]/50 rounded-xl p-4 mb-6 flex gap-3">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.75} className="w-5 h-5 text-[#C8973A] shrink-0 mt-0.5">
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
        </svg>
        <div>
          <p className="text-sm font-semibold text-[#C8973A]">Session timeout: 30 minutes</p>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
        <p className="text-xs font-bold font-medium text-slate-400 mb-5">Change Password</p>
        <form onSubmit={handleChangePassword} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-600 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#B8D9C8] placeholder-slate-500"
            />
          </div>
          <div>
          ...
          {success && <p className="text-xs text-[#2ab289] bg-green-900/30 border border-green-800 rounded-xl px-4 py-3">{success}</p>}

          <button type="submit" disabled={loading}
            className="w-full py-3 rounded-xl bg-[#145C44] hover:bg-[#145C44] text-white font-semibold text-sm transition-colors disabled:opacity-40">
            {loading ? (
