...
// ── ListStyle extension — list-style-type on bulletList / orderedList ─────────
...
// ── LineHeight extension — line-height on paragraph ───────────────────────────
  ...
  return (
    <button type="button" title={title} disabled={disabled}
      ...
      className={`flex items-center justify-center h-7 w-7 rounded text-sm transition-colors
        ${active ? 'bg-[#145C44] text-white' : 'text-slate-600 hover:bg-slate-100'}
        disabled:opacity-30`}>
      {children}
    </button>
  );
...
const selectCls = 'h-7 rounded border border-slate-200 bg-white text-xs text-slate-700 px-1.5 focus:outline-none focus:ring-1 focus:ring-green-600';
...
function Toolbar({ editor }: { editor: Editor | null }) {
  ...
  const bulletStyle = editor.getAttributes('bulletList').listStyleType ?? '';
  const numStyle    = editor.getAttributes('orderedList').listStyleType ?? '';
  ...
  return (
    <div className="flex flex-wrap items-center gap-0.5 px-3 py-2 border-b border-slate-200 bg-slate-50">
      ...
      {/* Font */}
      <select title="Font family" value={currentFont} className={selectCls}
      ...
      {/* Size */}
      <select title="Font size" value={currentSize} className={`${selectCls} w-16`}
        onChange={e => e.target.value
      ...
      {/* Line height */}
      <select title="Line spacing" value={currentLH} className={`${selectCls} w-[4.5rem]`}
        onChange={e => {
          if (e.target.value) editor.chain().focus().updateAttributes('paragraph', { lineHeight: e.target.value }).run();
          else editor.chain().focus().updateAttributes('paragraph', { lineHeight: null }).run();
        }}>
      ...
      {/* Bullet list + style */}
      <ToolBtn title="Bullet list" active={inBullet} onClick={() => editor.chain().focus().toggleBulletList().run()}>
        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
      ...
      <select
        ...
        onChange={e => editor.chain().focus().updateAttributes('bulletList', { listStyleType: e.target.value || null }).run()}
      >
      ...
      {/* Ordered list + style */}
      <ToolBtn title="Numbered list" active={inOrdered} onClick={() => editor.chain().focus().toggleOrderedList().run()}>
        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
      ...
      <select
        ...
        value={numStyle}
        disabled={!inOrdered}
        className={`${selectCls} w-[8rem]`}
        onChange={e => editor.chain().focus().updateAttributes('orderedList', { listStyleType: e.target.value || null }).run()}
      >
        <option value="">Style</option>
